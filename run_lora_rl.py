...
def eval(answer):
    # 5 points for answer tags
    # 5 points per prime factor after that
    points = 0
    if "<ans>" in answer and "</ans>" in answer: # good enough ig
        # change to regex
        points += 5
    # prime factors
    return points
...
### load data
### --- here we use a single task as an overfit test
tests = []
context = [
    {
        "role":
        ...
        "content": [
            {
                "type": "text",
                "text": "What are the prime factors of 19293?"
            },
        ],
    },
]
...
for i in range(1000):
    ...
    lora_params = jax.tree_util.tree_map(lambda p, g: p - lr*g, lora_params, grads) 
